...
export class Tx {
  ...
  public async sendMessage(
    ...
    const signedMessage = JSON.parse(
      this.transactionSignLotus(unsignedMessage, privateKey),
    )
  ...
  public transactionParse(cborMessage: string, network: Network) {
    ...
    if (Object.values(decoded).length < 10) {
      throw new Error(
        'The cbor message is missing some fields... please verify you have 9 fields',
      )
    }
   ...
   */
  public transactionSignLotus(
    unsignedMessage: string | Message,
    privateKey: PrivateKey,
  ): string {
    let message
